...
export default function CalendarPage() {
  ...
  const loadWorkouts = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        const data: WorkoutWithCompletion[] = await res.json();
        setWorkouts(data);
        // Set initial month to first workout's month
        if (data.length > 0) {
          const firstDate = new Date(data[0].workout.date + "T00:00:00");
          const now = new Date();
          // Use current month if it falls within the plan, otherwise use plan start
          const planStart = firstDate;
          const planEnd = new Date(
            data[data.length - 1].workout.date + "T00:00:00"
          );
